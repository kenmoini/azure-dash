...
- A live check on OpenShift ZTWIM. The Identity panel should show the SPIFFE ID as the federated `sub`, and the federated token's `iat`/`exp` should advance between two Identity page loads a few minutes apart (spiffe-helper rotation, re-read on every load); after the Entra access token's `exp` shown on the Identity page has passed, "Refresh all" should still succeed, which shows the callback re-reads the file for a new exchange rather than serving a cached access token. The cluster checks need real AKS and OpenShift environments; the build and render steps can be verified locally.
